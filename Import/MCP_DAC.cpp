...
void MCP_DAC::writeData(uint16_t data0, uint16_t data1)
{
  directWriteLow(_select);
  // digitalWrite(_select, LOW);
  mySPI->beginTransaction(_spi_settings);
  mySPI->transfer((uint8_t)((0x3000 | data0 >> 4) >> 8));
  mySPI->transfer((uint8_t)(data0 & 0xFF));
  directWriteHigh(_select);
  directWriteLow(_select);
  mySPI->transfer((uint8_t)((0xB000 | data1 >> 4) >> 8));
  mySPI->transfer((uint8_t)(data1 & 0xFF));
